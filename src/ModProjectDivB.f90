...
contains
  ...
  subroutine project_divb
    ...
    if(oktest_me)write(*,*) NameSub, ': Poisson solver info, nmatvec, resid:',&
         info, nmatvec, resid
...
!!! test_string=test_string(1:50)
    ...
    if(oktest)then
       ...
       if(abs(resid-divbmax_now)/(divbmax_now+1.e-12) > 0.1)then
          ...
          if(oktest_me)write(*,*) NameSub, ': resid,max(abs(divB))',&
               resid, divbmax_now
       end if
       ...
       if(pmin_new<0.5*pmin_old)then
          ...
          if(oktest_me)write(*,*) NameSub, ': new min(p)',pmin_new
       endif
