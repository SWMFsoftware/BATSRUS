...
case 1 of
  ...
  f eq 'csound' or strmid(f,0,4) eq 'mach' or f eq 'e' or $
  ...
  f eq 'A' or f eq 'A_r' or f eq 'AA' or f eq 'AA_r' or f eq 'B':begin
     bx=b1
     by=b2
     if f eq 'A_r' or f eq 'AA_r' then begin
        ; For axial symmetry Cartesian_curl(r*A)=(B_r*r,B_z*r)
        bx=r*bx
        by=r*by
     endif
     result=dblarr(n1,n2)
     if f eq 'A' or f eq 'A_r' then begin
     ...
     if f eq 'B' then result=smooth(result,4)
  end
  ...
  f eq 'bern3': result=gamma*e/rho+(1-gamma)/2*(m1^2+m2^2+m3^2)/rho^2 $
                       -0.5/(sqrt(r^2+z^2)-1)
