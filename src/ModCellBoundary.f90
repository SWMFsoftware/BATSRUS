...
contains
  ...
  subroutine BC_cont(iVarStart,iVarLast)
    ...
    State_VGB(iVarStart:iVarLast, &
         imin1g:imax1g,jmin1g:jmax1g,kmin1g:kmax1g,iBLK) = &
         State_VGB(iVarStart:iVarLast, &
         imin1p:imax1p,jmin1p:jmax1p,kmin1p:kmax1p,iBLK)
    State_VGB(iVarStart:iVarLast, &
         imin2g:imax2g,jmin2g:jmax2g,kmin2g:kmax2g,iBLK) = &
         State_VGB(iVarStart:iVarLast, &
         imin1p:imax1p,jmin1p:jmax1p,kmin1p:kmax1p,iBLK)
  ...
  subroutine BC_symm(iVarStart,iVarLast)
    ...
    State_VGB(iVarStart:iVarLast, &
         imin2g:imax2g,jmin2g:jmax2g,kmin2g:kmax2g,iBLK) = &
         State_VGB(iVarStart:iVarLast, &
         imin2p:imax2p,jmin2p:jmax2p,kmin2p:kmax2p,iBLK)
  ...
  subroutine BC_solar_wind(time_now)
    ...
    do k = kmin1g, kmax1g 
       z = Xyz_DGB(z_,1,1,k,iBLK)
       do j = jmin1g, jmax2g
          y = Xyz_DGB(y_,1,j,1,iBLK)
          do i = imin1g, imax2g, sign(1,imax2g-imin1g)

             ! x= Xyz_DGB(x_,i,j,k,iBLK) ! for cell based BC this would be best
       ...
       do k = 1, nK; do j = 1, nJ
          ...
          do iVar = iVarFirst, iVarLast
             ...
             Coef = 2/sqrt( (3*OpacityRosselandSi_W(iWave) &
                  /          Si2No_V(UnitX_) * CellSize_DB(x_,iBLK))**2 &
    ...
    case(3,4)
       do k = 1, nK; do i = 1, nI
          ...
          do iVar = iVarFirst, iVarLast
             iWave = iVar - iVarFirst + 1
             Coef = 2/sqrt( (3*OpacityRosselandSi_W(iWave) &
                  /          Si2No_V(UnitX_) * CellSize_DB(y_,iBLK))**2 &
    ...
    case(5,6)
       do j = 1, nJ; do i = 1, nI
          ...
          do iVar = iVarFirst, iVarLast
             iWave = iVar - iVarFirst + 1
             Coef = 2/sqrt( (3*OpacityRosselandSi_W(iWave) &
                  /          Si2No_V(UnitX_) * CellSize_DB(z_,iBLK))**2 &
