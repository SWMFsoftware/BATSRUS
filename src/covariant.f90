...
subroutine set_covar_cartesian_geometry(iBLK)
  ...
  VolumeInverse_I((iBLK-1)*nIJK+1:iBLK*nIJK)=cOne/(dx*dy*dz)
...
subroutine set_covar_spherical_geometry(iBLK)
  ...
  use ModGeometry,ONLY:x_BLK,y_BLK,z_BLK,R_BLK,dx_BLK,dy_BLK,dz_BLK,&
       DoFixExtraBoundary_B,XyzStart_BLK,XyzMin_D,XyzMax_D,VolumeInverse_I
  use ModCovariant
  use ModNumConst
  implicit none
...
subroutine covariant_force_integral(iVolumeCounter,i,j,k,iBLK,Fai_S)
  ...
  use ModGeometry,ONLY: VolumeInverse_I
  implicit none
  ...
  VInv=VolumeInverse_I(iVolumeCounter)
...
subroutine covariant_gradient(iBlock, Var_G,&     
  ...
  if(.not.body_BLK(iBlock)) then
     ...
     do k=1,nK; do j=1,nJ; do i=1,nI
        iVolumeCounter=iVolumeCounter+1
        VInvHalf=chalf*VolumeInverse_I(iVolumeCounter)
     ...
     do k=1,nK;  do j=1,nJ;  do i=1,nI
        iVolumeCounter=iVolumeCounter+1
        VInvHalf=&
             chalf*VolumeInverse_I(iVolumeCounter)*OneTrue_G(i,j,k)
...
subroutine covariant_curlb(i,j,k,iBLK,CurlB_D)
  ...
  VInvHalf=chalf*VolumeInverse_I((iBLK-1)*nIJK+i+nI*(j-1+ nJ*(k-1)))
