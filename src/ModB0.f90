    ...
    if(UseB0MomentumFlux)then
       ...
       do k = 1, nK; do j = 1, nJ; do i = 1, nI
          B0MomentumSource_DC(:,i,j,k) =                                &
               + B0_DX(:,i+1,j,k)*sum(B0_DGB(:,i,j,k,iBlock)*           &
               FaceNormal_DDFB(:,1,i+1,j,k,iBlock))                     &
               - FaceNormal_DDFB(:,1,i+1,j,k,iBlock)*                   &
               sum(B0_DX(:,i+1,j,k)*B0_DGB(:,i,j,k,iBlock))             &
               - B0_DX(:,i  ,j,k)*sum(B0_DGB(:,i,j,k,iBlock)*           &
               FaceNormal_DDFB(:,1,i,j,k,iBlock))                       &
               + FaceNormal_DDFB(:,1,i,j,k,iBlock)*                     &
               sum(B0_DX(:,i  ,j,k)*B0_DGB(:,i,j,k,iBlock))             &
               + B0_DY(:,i,j+1,k)*sum(B0_DGB(:,i,j,k,iBlock)*           &
               FaceNormal_DDFB(:,2,i,j+1,k,iBlock) )                    &
               - FaceNormal_DDFB(:,2,i,j+1,k,iBlock)*                   &
               sum(B0_DY(:,i,j+1,k)*B0_DGB(:,i,j,k,iBlock))             &
               - B0_DY(:,i,j  ,k)*sum(B0_DGB(:,i,j,k,iBlock)*           &
               FaceNormal_DDFB(:,2,i,j  ,k,iBlock) )                    &
               + FaceNormal_DDFB(:,2,i,j  ,k,iBlock)*                   &
               sum(B0_DY(:,i,j  ,k)*B0_DGB(:,i,j,k,iBlock))

          if(nDim == 3) B0MomentumSource_DC(:,i,j,k) =                  &
               B0MomentumSource_DC(:,i,j,k)                             &
               + B0_DZ(:,i,j,k+1)*sum(B0_DGB(:,i,j,k,iBlock)*           &
               FaceNormal_DDFB(:,3,i,j,k+1,iBlock))                     &
               - FaceNormal_DDFB(:,3,i,j,k+1,iBlock)*                   &
               sum(B0_DZ(:,i,j,k+1)*B0_DGB(:,i,j,k,iBlock))             &
               - B0_DZ(:,i,j,k  )*sum(B0_DGB(:,i,j,k,iBlock)*           &
               FaceNormal_DDFB(:,3,i,j,k  ,iBlock) )                    &
               + FaceNormal_DDFB(:,3,i,j,k  ,iBlock)*                   &
               sum(B0_DZ(:,i,j,k  )*B0_DGB(:,i,j,k,iBlock))
