...
module ModUser
   ...
   character (len=*), parameter :: NameUserFile = "ModUserAncientMoonImpact.f90"
   character (len=*), parameter :: NameUserModule = 'Ancient Moon with Asteroid Impact (based on Mercury), Isaac Narrett v1.0'
 
 
   real :: PlanetDensity=-1., PlanetPressure=-1., PlanetRadius=-1., PlaRad=1.
 ...
 !!! in dimensional units.
   ...
   subroutine user_read_inputs
     ...
     integer :: iLayer=0,RootBlocks=0,CellsPerBlock=0,RefineLevel=1
     real :: i=0.0,rMinUser=0.0,rMaxUser=0.0
   ...
   subroutine user_set_ics(iBlock)
     ...
     !do k=1,nK; do j=1,nJ; do i=1,nI
     !    if(Xyz_DGB(z_,i,j,k,iBlock) > -1.0)then!PlanetRadius) CYCLE
   ...
   subroutine user_set_cell_boundary(iBlock, iSide, TypeBc, IsFound)
     ...
     !if((nIteration < 1 .or. tSimulation == 0.0) .and. PlanetRadius == 1.0)&
     !    call move_user_surface(iBlock, PlanetRadius)
     !if(PlanetRadius == 1.0) then 
     !    do i = 1, nI
     !       if((r_GB(i,1,1,iBlock) >= rSurface) .and. &
     !          (r_GB(i-1,1,1,iBlock) < rSurface))then
     !          PlanetRadius = rSurface + 2*CellSize_DB(1,iBlock)   
     !       end if
     !    end do;
     !    write(*,*)PlanetRadius
     ! end if
     ...
     if(rMin_B(iBlock) <= PlanetRadius) then
        ...
        if(r_GB(MaxI,1,1,iBlock) >= PlanetRadius)then
           ...
           do k = 1, nK; do j = 1, nJ
              ! Get radial velocity
              !write(*,*)'hey1',i,r_GB(i,j,k,iBlock),PlanetRadius
              !write(*,*)'hey2',i-1,r_GB(i-1,j,k,iBlock),PlanetRadius
              !write(*,*)'hey3',i-2,r_GB(i-2,j,k,iBlock),PlanetRadius
	      r_D = Xyz_DGB(x_:z_,i,j,k,iBlock)/ r_GB(i,j,k,iBlock)
 
              RhoUr = dot_product(State_VGB(RhoUx_:RhoUz_,i,j,k,iBlock),r_D)
              if(RhoUr > 0.0 .or. (State_VGB(Rho_,i,j,k,iBlock) > 1000.* SolarWindRho)) then
              ...
              if(RhoUr>0 .or. (State_VGB(Rho_,i,j,k,iBlock) > 1000.* SolarWindRho)) then
                 do iG = i-nG, i-1
                    !State_VGB(Hyp_,iG,j,k,iBlock) = 0.0!State_VGB(Hyp_,i,j,k,iBlock)
              ...
              if(UseImpact .eqv. .true.) then
                  ...
                  if ((tSimulation < (250.)) .and.  &
                        ...
                        do iG = i-nG, i-1
                           ...
                           do iFluid = 1, nFluid
                              call select_fluid(iFluid)
                              State_VGB(iUx:iUz,iG,j,k,iBlock) = 0.0!(((/x,y,z/)* 1000.0 * Si2No_V(UnitU_))& 
                                                                        !* State_VGB(Rho_,iG,j,k,iBlock))
                           end do
                           State_VGB(P_,iG,j,k,iBlock)  = 4000.*Si2No_V(UnitTemperature_)* &
                           (State_VGB(Rho_,iG,j,k,iBlock) / MassIon_I(1)) 
                           !VarsGhostFace_V(Bx_:Bz_) = 0.0 
                           !State_VGB(Bx_:Bz_,iG,j,k,iBlock) = 0.0!-B0_DGB(:,iG,j,k,iBlock) + ((/1.0,0.0,0.0/) * 1e-9) * Si2No_V(UnitB_)
                           !State_VGB(Bx_:Bz_,i,j,k,iBlock) = 0.0
                        end do
                  end if
               end if 
   ...
   !============================================================================
   !subroutine move_user_surface(iBlock, PR)
   !  use BATL_grid, ONLY: CellSize_DB
   !  use ModGeometry,   ONLY: r_GB
   !  use ModMain, ONLY: nI
   ...
   !  character(len=*), parameter:: NameSub = 'move_user_surface'
     !--------------------------------------------------------------------------
   !  do i=1,nI;
   !   if((r_GB(i-1,1,1,iBlock) < PlaRad) .and. &
   !   (r_GB(i,1,1,iBlock) >= PlaRad))then
   !      PR = PlaRad + 2*CellSize_DB(1,iBlock)
   !      EXIT
   !   end if
   !  end do
 
   !end subroutine move_user_surface
    !==============================================================================
end module ModUser
 !==============================================================================
