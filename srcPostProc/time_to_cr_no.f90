...
! 360   Carrington coordinate (0 corresponds to end time, 360 to start time.)
program time_tocrno
  use ModUT
  use ModConst
  use ModTimeConvert
  ...
  call CON_recalc(iTimeIn_I(1), iTimeIn_I(2), iTimeIn_I(3), iTimeIn_I(4), &
       iTimeIn_I(5),iTimeIn_I(6))
  ...
  ! Now we have a time which corrsponds to the given Carrington Longitude
  ! in the rotation 2008
...
contains
  ...
  subroutine solve_time
    ...
    call CON_recalc(iTimeL_I(1),iTimeL_I(2),iTimeL_I(3),iTimeL_I(4), &
         iTimeL_I(5),iTimeL_I(6))
    LongitudeL=CarringtonLongitude

    call time_real_to_int(TimeR,iTimeR_I)
    call CON_recalc(iTimeR_I(1),iTimeR_I(2),iTimeR_I(3),iTimeR_I(4), &
         iTimeR_I(5),iTimeR_I(6))
    ...
    do while( any( iTimeL_I(1:5)/=iTimeR_I(1:5)).and.TimeR-TimeL>10.0)
       ...
       call CON_recalc(iTime_I(1),iTime_I(2),iTime_I(3),iTime_I(4), &
            iTime_I(5),iTime_I(6))
