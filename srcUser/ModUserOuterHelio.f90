...
! 02/08/2018 adding possibilty of using user boundaries for other faces besides
!   east
...
! 07/2022 Merav Opher added conditions needed for Cold Cloud
  ...
  use ModUserEmpty,                                     &
       ...
       IMPLEMENTED13 => user_init_point_implicit,       &  ! why needed ?
  ...
  character (len=*), parameter :: &
       NameUserModule = 'Outer Heliosphere with 4 neutrals and 1 or 2 ion fluids with or without electron pressure, Opher & Toth'

  ! Zieger Named indexes for fluids
  integer, parameter :: SWH_ = 1, Ion_ = 1, Pu3_ = min(nFluid,2), Neu_ = min(nFluid,IonLast_+1), &
       Ne2_ = min(nFluid,IonLast_+2), Ne3_ = min(nFluid,IonLast_+3), Ne4_= min(nFluid,IonLast_+4)

  logical :: UseSource_I(SWH_:Ne4_) = .true.
  logical :: UsePhotoion = .false.
  logical :: ColdCloud = .false.
  logical :: InitializeCloud = .false.
...
contains
    ...
    do
       ...
       case("#SOLARWINDH")
          ...
          if(UseElectronPressure)then
             call read_var('SWH_Te_dim'  ,SWH_Te_dim)
          end if
       case("#SOLARWINDFAST")
       ...
       case("#VLISW")
          ...
          if(UseElectronPressure)then
             call read_var('VLISW_Te_dim' ,VLISW_Te_dim)
          end if
       case("#PICKUPION3")
       ...
       case("#COLDCLOUD")
          call read_var('UseColdCloud', ColdCloud)
          call read_var('InitializeCloud', InitializeCloud)
  ...
  subroutine user_set_cell_boundary(iBlock, iSide, TypeBc, IsFound)
    ...
    do k = kMin, kMax; do j = jMin, jMax; do i = iMin, iMax
       State_VGB(SWHRho_,i,j,k,iBlock)=VLISW_rho
       State_VGB(SWHRhoUx_,i,j,k,iBlock)=VLISW_rho*VLISW_Ux
       State_VGB(SWHRhoUy_,i,j,k,iBlock)=VLISW_rho*VLISW_Uy
       State_VGB(SWHRhoUz_,i,j,k,iBlock)=VLISW_rho*VLISW_Uz
       State_VGB(Bx_,i,j,k,iBlock)=VLISW_Bx
       State_VGB(By_,i,j,k,iBlock)=VLISW_By
       State_VGB(Bz_,i,j,k,iBlock)=VLISW_Bz
       State_VGB(SWHP_,i,j,k,iBlock)=VLISW_p
       if(UseElectronPressure) State_VGB(Pe_,i,j,k,iBlock)=VLISW_Pe

       if(.not.IsMhd)then
          State_VGB(Pu3Rho_,  i,j,k,iBlock) = 1E-5*VLISW_rho
          State_VGB(Pu3RhoUx_,i,j,k,iBlock) = 1E-5*VLISW_rho*VLISW_Ux
          State_VGB(Pu3RhoUy_,i,j,k,iBlock) = 1E-5*VLISW_rho*VLISW_Uy
          State_VGB(Pu3RhoUz_,i,j,k,iBlock) = 1E-5*VLISW_rho*VLISW_Uz
          State_VGB(Pu3P_,    i,j,k,iBlock) = 1E-5*VLISW_p
       end if
  ...
  subroutine user_set_ics(iBlock)
    ...
    do k=MinK,MaxK; do j=MinJ,MaxJ; do i=MinI,MaxI
       ...
       if(.not.IsMhd)then
          ...
          if(UseElectronPressure)then
             State_VGB(Pe_,i,j,k,iBlock)   = SWH_Pe   * (rBody/r)**(2*Gamma)
          end if
       else
       ...
       !! for studies of Heliosphere in cold cloud
          if (r > rBody) then
             ...
             State_VGB(Rho_,i,j,k,iBlock) = VLISW_rho
             State_VGB(P_,i,j,k,iBlock)   = VLISW_p

             State_VGB(RhoUx_,i,j,k,iBlock) = VLISW_Ux*VLISW_rho
             State_VGB(RhoUy_,i,j,k,iBlock) = VLISW_Uy*VLISW_rho
             State_VGB(RhoUz_,i,j,k,iBlock) = VLISW_Uz*VLISW_rho
          endif
       end if
       ...
       if(UseNeutralFluid)then
         ...
         ! PopI for studies of Heliosphere in cold cloud
            State_VGB(NeuRho_,i,j,k,iBlock)  = VLISW_rho
            State_VGB(NeuP_,i,j,k,iBlock) =   VLISW_p
            State_VGB(NeuRhoUx_,i,j,k,iBlock)= VLISW_Ux*VLISW_rho
            State_VGB(NeuRhoUy_,i,j,k,iBlock)= VLISW_Uy*VLISW_rho
            State_VGB(NeuRhoUz_,i,j,k,iBlock)= VLISW_Uz*VLISW_rho
         else
         ! PopI
       ...
       if(DoTestCell)then
          ...
          ! Zieger
          if(.not.IsMhd)then
             write(*,*)NameSub,' SWHRhoU_D =',State_VGB(SWHRhoUx_:SWHRhoUz_,i,j,k,iBlock)
  ...
  subroutine user_action(NameAction)
    ...
    if(.not.IsMhd)then
       ...
       write(*,*)
       if(UseElectronPressure)then
          write(*,StringFormat) 'VLISW_Pe_dim [nPa]: ',VLISW_Pe_dim,'VLISW_Pe:',VLISW_Pe
          write(*,'(10X,A19,F15.6)') 'VLISW_Te_dim[K]: ',VLISW_Te_dim
          write(*,StringFormat) 'SWH_Te_dim   [   K]:',SWH_Te_dim,'SWH_Pe:',SWH_Pe
       end if
    end if
    !------------------------------------------------------------------
  ...
  subroutine user_calc_sources_expl(iBlock)
    ...
    real, dimension(nFluid,3) :: u_D
    real, dimension(Neu_:Ne4_,5) :: SrcLookI_I, SrcLookN_I
       ...
       if(allocated(ExtraSource_ICB))then
          ...
          do k = 1, nK; do j = 1, nJ; do i = 1, nI
             ...
             if(.not.IsMhd)then
                ...
                Source_VC(SWHRhoUx_:RhoUz_,i,j,k) = Source_VC(SWHRhoUx_:SWHRhoUz_,i,j,k) + sRhoU_D
                Source_VC(SWHEnergy_,i,j,k) = Source_VC(SWHEnergy_,i,j,k) + sEnergy
                Source_VC(SWHP_,i,j,k) = Source_VC(SWHP_,i,j,k) &
                     + GammaMinus1*( sEnergy - sum(uSWH_D*sRhoU_D) + 0.5*USWH2*sRho )
                ...
                Source_VC(Pu3RhoUx_:Pu3RhoUz_,i,j,k) = Source_VC(Pu3RhoUx_:Pu3RhoUz_,i,j,k) + sRhoU_D
                Source_VC(Pu3Energy_,i,j,k) = Source_VC(Pu3Energy_,i,j,k) + sEnergy
                Source_VC(Pu3P_,i,j,k) = Source_VC(Pu3P_,i,j,k) &
                     + GammaMinus1*( sEnergy - sum(uPu3_D*sRhoU_D) + 0.5*UPu32*sRho )
                ...
                Source_VC(RhoUx_:RhoUz_,i,j,k) = Source_VC(RhoUx_:RhoUz_,i,j,k) + sRhoU_D
             ...
             if(DoTest .and. i==iTest .and. j==jTest .and. k==kTest)then
                write(*,*) 'SWH_Rho = ', State_V(Rho_)
                write(*,*) 'U2 =', USWH2
                write(*,*) 'SWH_P   = ', State_V(p_)
                ...
                ! Zieger
                if(.not.IsMhd)then
                   write(*,*) ' PUI Density Source =', ExtraSource_ICB(6,i,j,k,iBlock)
                   write(*,*) ' PUI MVx Source =', ExtraSource_ICB(7,i,j,k,iBlock)
                   write(*,*) ' PUI Energy Source =', ExtraSource_ICB(10,i,j,k,iBlock)
                end if
             endif
    ...
    cth = 2.0*cBoltzmann/cProtonMass
    ...
    do k=1,nK; do j=1,nJ; do i=1,nI
       ...
       ! u_D  = State_V(iRhoUx_I:iRhoUz_I)/State_V(iRho_I)*No2Si_V(UnitU_)
       u_D(:,1) = Ux_I*No2Si_V(UnitU_)
       u_D(:,2) = Uy_I*No2Si_V(UnitU_)
       u_D(:,3) = Uz_I*No2Si_V(UnitU_)
       ...
       if(IsMhd)then
          ...
          Temp_I(SWH_) = (State_V(SWHP_)/(2*State_V(SWHRho_)+State_V(PU3Rho_))) &
       ...
       if(iTableChargeExchange > 0) then
          do iFluid = Neu_, Ne4_
             call get_charge_exchange(Rho_D(Ion_), UthS_I(Ion_), u_D(Ion_,:), &
                  Rho_D(iFluid), UthS_I(iFluid), u_D(iFluid,:), &
                  SrcLookI_I(iFluid,:),SrcLookN_I(iFluid,:))
          enddo

          where(UseSource_I(Neu_:)) SrcLookRhoI_I    = SrcLookI_I(Neu_:,1)
          where(UseSource_I(Neu_:)) SrcLookRhoUxI_I  = SrcLookI_I(Neu_:,2)
          where(UseSource_I(Neu_:)) SrcLookRhoUyI_I  = SrcLookI_I(Neu_:,3)
          where(UseSource_I(Neu_:)) SrcLookRhoUzI_I  = SrcLookI_I(Neu_:,4)
          where(UseSource_I(Neu_:)) SrcLookEnergyI_I = SrcLookI_I(Neu_:,5)

          where(UseSource_I(Neu_:)) SrcLookRhoN_I    = SrcLookN_I(Neu_:,1)
          where(UseSource_I(Neu_:)) SrcLookRhoUxN_I  = SrcLookN_I(Neu_:,2)
          where(UseSource_I(Neu_:)) SrcLookRhoUyN_I  = SrcLookN_I(Neu_:,3)
          where(UseSource_I(Neu_:)) SrcLookRhoUzN_I  = SrcLookN_I(Neu_:,4)
          where(UseSource_I(Neu_:)) SrcLookEnergyN_I = SrcLookN_I(Neu_:,5)
          ...
          if(.not.IsMhd)then
             ! For PU3
             where(UseSource_I(Neu_:)) &
                  SigmaPu3_I =((2.2835E-7 - (1.062E-8)*log(UStarMPu3_I*100.))**2)*(1.E-4)
                  SigmaNPu3_I =((2.2835E-7 - (1.062E-8)*log(UStarPu3_I*100.))**2)*(1.E-4)
          end if
       ...
       if(UsePhotoion)then
          ...
          ! r0 = 1 AU
          where(UseSource_I(Neu_:)) &
               RatePh_I = 8E-8*(1/(r+1e-10))**2*State_V(iRho_I(Neu_:))* No2Si_V(UnitT_)
         ...
         if (iFluid == iFluidProduced_C(i,j,k)) then
            ...
            if (iTableChargeExchange < 0) then
               ...
               if(.not.IsMhd)then
                  ...
                  Source_V(iRho)    = sum(I0xp_I)  - I0xp_I(iFluid) - I0xpPh_I(iFluid)
                  Source_V(iRhoUx)  = sum(JxpUx_I) - JpxUx_I(iFluid) - JxpUxPh_I(iFluid)
                  Source_V(iRhoUy)  = sum(JxpUy_I) - JpxUy_I(iFluid) - JxpUyPh_I(iFluid)
                  Source_V(iRhoUz)  = sum(JxpUz_I) - JpxUz_I(iFluid) - JxpUzPh_I(iFluid)
                  Source_V(iEnergy) = sum(Kxp_I)   - Kpx_I(iFluid) - KxpPh_I(iFluid)
               endif
            else
      ...
      if (iTableChargeExchange < 0) then
         ...
         if(.not.IsMhd)then
            ...
            if(UseSource_I(Ion_))then
               !! Source_V(RhoUx_) = sum( QmpxUx_I )
               !! Source_V(RhoUy_) = sum( QmpxUy_I )
               !! Source_V(RhoUz_) = sum( QmpxUz_I )
...
! cold cloud
    ...
    do k=1,nK; do j=1,nJ; do i=1,nI
       ...
       if(.not.IsMhd)then
          if(UseElectronPressure) then
             p = State_VGB(SWHp_,i,j,k,iBlock) + State_VGB(Pu3p_,i,j,k,iBlock) + State_VGB(Pe_,i,j,k,iBlock)
          else
          ...
          Ux = (State_VGB(RhoUx_,i,j,k,iBlock) + State_VGB(Pu3RhoUx_,i,j,k,iBlock)) &
               /(State_VGB(Rho_,i,j,k,iBlock) + State_VGB(Pu3Rho_,i,j,k,iBlock))
          Uy = (State_VGB(RhoUy_,i,j,k,iBlock) + State_VGB(Pu3RhoUy_,i,j,k,iBlock)) &
               /(State_VGB(Rho_,i,j,k,iBlock) + State_VGB(Pu3Rho_,i,j,k,iBlock))
          Uz = (State_VGB(RhoUz_,i,j,k,iBlock) + State_VGB(Pu3RhoUz_,i,j,k,iBlock)) &
               /(State_VGB(Rho_,i,j,k,iBlock) + State_VGB(Pu3Rho_,i,j,k,iBlock))
          U2 = (Ux**2 + Uy**2 + Uz**2)
...
! for cold cloud
          ...
          MachMagneto2 = U2/((1.E-10)+(Gamma*T_ave)+(B2*InvRho)) ! Zieger Here you need the speed of the total fluid (U2)
          ! Square of Mach number
          Mach2  = U2/(Gamma*p*InvRho)
          ! Square of PUI Mach number (using total fluid speed) ! Zieger This Mach number has no physical meaning
          MachPUI2 = U2/(Gamma*pPUI*InvRhoPUI)
          ! Square of Solar Wind Mach number (using total fluid speed) ! Zieger This Mach number has no physical meaning
          MachSW2 = U2/(Gamma*pSW*InvRhoSW)
          ! Square of Solar Wind Mach number (using SWH speed) ! Zieger This Mach number has no physical meaning
...
!!! july12 use sonic Mach number - Berci
          if(.not.ColdCloud)then
             ...
             if(InitializeCloud)then
                ...
                if (TempPop1LimitDim > TempDim .and. rhoPop4LimitDim > rhoDim) then
                   ! Outside the bow shock
                   iFluidProduced_C(i,j,k) = Ne4_
                elseif (TempPop1LimitDim > TempDim .and. rhoDim > rhoPop4LimitDim) then
                   ! Outside the heliopause
                   iFluidProduced_C(i,j,k) = Neu_
                elseif(TempDim > TempPop1LimitDim .and. uPop1LimitDim**2 > U2Dim)then
                   ! Heliosheath
                   iFluidProduced_C(i,j,k) = Ne2_
                elseif(TempDim > TempPop1LimitDim .and. U2Dim > uPop1LimitDim**2)then
                   ! Inside termination shock
...
!!!    call test_stop(NameSub, DoTest)
  ...
  subroutine user_init_session
    ...
    if(IsMhd)then
       ...
       VLISW_p   = VLISW_T_dim*Io2No_V(UnitTemperature_)*VLISW_rho
       VLISW_Pe   = VLISW_Te_dim*Io2No_V(UnitTemperature_)*VLISW_rho
    end if
    ...
    if(.not.IsMhd)then
       ...
       ! For SWH_p, see eq. 1 from Opher et al. (2020)
       SWH_p = (2.*SWH_rho + PU3_rho)*SWH_T_dim*Io2No_V(UnitTemperature_)
       if(UseElectronPressure)then
          SWH_Pe = SWH_Te_dim*Io2No_V(UnitTemperature_)*(SWH_rho + PU3_rho)
       end if
    end if
    ...
    if(UseNeutralFluid)then
       ...
       pNeuWindDim = No2Io_V(UnitP_)/Gamma &
            *(RhoNeuWindDim/SWH_rho_dim)*(TempNeuWindDim /SWH_T_dim)
