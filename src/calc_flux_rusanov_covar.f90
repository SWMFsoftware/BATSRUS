  ...
  do iStart=iBegin,iEnd,MaxStrip
     ...
     do k=kBegin,kEnd
        ...
        do j=jBegin,jEnd
           ...
           do i=1,nStrip
              iStrip=i+iStart-1
              Flux_VY(1:Energy_,iStrip,j,k) = Full_Flux(1:Energy_,i) 
...
subroutine get_flux_mhdRusanov_covar(nStrip)
  ...
  real, dimension(MaxStrip) :: a2,Misc1_I,discr, mP_lf, mP_rf, BFull2_I
  ...
  do i=1,nStrip
     ...
     v_cright_hf(i) =cHalf*abs(v_Un_lf(i)+v_Un_rf(i)) + discr(i)         
     v_cleft_hf(i)  =v_cright_hf(i)
     v_max_hf(i)    =v_cright_hf(i)              ! for further calculation of CFL
  end do
  ...
  do i=1,nStrip
     do n=1,nFlux
        Full_Flux(n,i) = Fl_lf(n,i)+(Fl_rf(n,i)-Fl_lf(n,i))*cHalf 
     end do
  end do
