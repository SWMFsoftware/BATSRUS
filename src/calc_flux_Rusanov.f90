...
subroutine option_rusanovflux(TrueOption,NameOption)
  ...
  TrueOption  = .true.
  NameOption  = 'RUSANOV FLUX De Zeeuw, Toth, Sokolov 1.4'
...
subroutine calc_flux_Rusanov(DoResChangeOnly)
  ...
  real, dimension(MaxStrip) :: v_cleft_hf, v_cright_hf
...
Contains
  ...
  subroutine get_fluxes
    ...
    do i=1,nStrip
       ...
       do n=1,nFlux
          Full_Flux(n,i) = cHalf*fA*((L_Flux(n,i)+R_Flux(n,i)) &
               - v_max_hf(i)*delta_U(n,i))
       end do
    ...
    do i=1,nStrip
       ...
       case (1)
          ...
          UDotFA_X(iStrip,j,k)     = cHalf*fA*(v_Un_lf(i)+v_Un_rf(i))
          EDotFA_X(iStrip,j,k)     = &               !^CFG IF BORISCORR
               cHalf*fA*(v_En_lf(i)+v_En_rf(i))      !^CFG IF BORISCORR
       case (2)
          !\
          ! y face
          !/
          ...
          UDotFA_Y(iStrip,j,k)     = cHalf*fA*(v_Un_lf(i)+v_Un_rf(i))
          EDotFA_Y(iStrip,j,k)     = &               !^CFG IF BORISCORR
               cHalf*fA*(v_En_lf(i)+v_En_rf(i))      !^CFG IF BORISCORR
       case (3)
          !\
          ! z face
          ...
          UDotFA_Z(iStrip,j,k)     = cHalf*fA*(v_Un_lf(i)+v_Un_rf(i))
          EDotFA_Z(iStrip,j,k)     = &              !^CFG IF BORISCORR
               cHalf*fA*(v_En_lf(i)+v_En_rf(i))     !^CFG IF BORISCORR
       end select
    end do
