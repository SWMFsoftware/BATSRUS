...
module ModUser
  ...
  character (len=*), parameter :: NameUserFile = "ModUserSaturn3sp.f90"
...
contains
  ...
  subroutine user_init_session
    ...
    do iBoundary = xMinBc_, zMaxBc_
       FaceState_VI(RhoH_, iBoundary)   = SolarWindRho
       FaceState_VI(RhoH2O_, iBoundary) = 1e-6*SolarWindRho
       FaceState_VI(RhoN_, iBoundary)   = 1e-6*SolarWindRho
    end do
  ...
  subroutine user_set_ics(iBlock)
    ...
    where(r_GB(:,:,:,iBlock)<2.0*Rbody)
       ...
       State_VGB(RhoH2O_,:,:,:,iBlock) = 1e-6*SolarWindRho
       State_VGB(RhoN_,:,:,:,iBlock)   = 1e-6*SolarWindRho
    elsewhere
       State_VGB(RhoH_,:,:,:,iBlock)   = SolarWindRho
  ...
  subroutine user_calc_sources_expl(iBlock)
    ...
    real, dimension(1:nI,1:nJ,1:nK):: &
         Srho_C,SrhoH_C,SrhoHC2O_C,SrhoN_C,SrhoUx_C,SrhoUy_C,SrhoUz_C,&
         SBx_C,SBy_C,SBz_C,SP_C,SE_C
    ...
    Source_VC(rhoH_   ,:,:,:) = SrhoH_C   + Source_VC(rhoH_   ,:,:,:)
    Source_VC(rhoH2O_ ,:,:,:) = SrhoHC20_C + Source_VC(rhoH2O_ ,:,:,:)
    Source_VC(rhoN_   ,:,:,:) = SrhoN_C   + Source_VC(rhoN_   ,:,:,:)
  ...
  contains
    ...
    subroutine user_sources(iBlock)
      ...
      !   SBx_C,SBy_C,SBz_C:  Souce terms for the magnetic field equations
      ...
      if (UseMassLoading) then
         ...
         do k=1,nK; do j=1,nJ do i=1,nI
            ...
            ! Do the rotations. Note that we want to compute the positions in
            ! the SMG coordinate system. However, we need to compute the
            ...
            if (((xSMG(3) < 2.0) .and.    &
               ...
               !
               ! Since Richardson gives a rate of about 1.3e27 s^-1 we multiply
               ! by a factor of 2 to get a number close to his.  This means that
               ...
               ! use to control the mass addition rate. A factor of 1.0
               ! corresponds to a rate of rhodot0 = 7.2e-4 amu/cm^3/s. A factor
               ! of 2.0 obviously corresponds to twice this and therefore
               ! corresponds to the rates in the table above!
               !
               !-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
               !-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
               !-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-
               ...
               CXsource = 0.6*((-0.0330 + 0.1636*Rxy)*1E-8)              &
                    *n0*CXsource*exp(-(xSMG(3)**2)/ Hneutral**2)
               ...
               ! This is used in a formula to get the recombination rate in s.
               ...
               Telectron = (16.6*cProtonMass/cBoltzmann)*&
                    ...
                    No2Si_V(UnitP_))/&
                    (State_VGB(rho_,i,j,k,iBlock)*No2Si_V(UnitRho_))*Tfrac
               ...
               ! No2Si_V(UnitRho_)/No2Si_V(UnitT_) to get the
               ...
               ! now load the source terms into the right hand side. Do
               ! rho as with single species, being the total mass. Do the
               ! SrhoHC20_C since this is just the water part.

               Srho(i,j,k)_C   = Srho_C(i,j,k) &
                    + (rhodot - Alpha_rec*State_VGB(rhoH2O_,i,j,k,iBlock))
               SrhoHC20_C(i,j,k) = SrhoHC20_C(i,j,k) &
                    + (rhodot - Alpha_rec*State_VGB(rhoH2O_,i,j,k,iBlock))
            ...
            if (Dtorus < 10.0) then
               ...
               ! now load the source terms into the right hand side. Do
               ! rho as with single species, being the total mass. Do the
               ! SrhoN_C since this is just the water part.

               Srho_C(i,j,k)   = Srho_C(i,j,k) &
                    + (rhodot - Lterm*State_VGB(rhoN_,i,j,k,iBlock))
               SrhoN_C(i,j,k)  = SrhoN_C(i,j,k)  &
                    + (rhodot - Lterm*State_VGB(rhoN_,i,j,k,iBlock))
               SrhoUx_C(i,j,k) = SrhoUx_C(i,j,k) + &
         ...
         end do; end do; end do ! end the i,j,k loop
