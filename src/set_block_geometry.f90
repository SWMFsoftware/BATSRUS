...
subroutine set_block_geometry_obsolete(iBLK)
...
end subroutine set_block_geometry_obsolete
...
subroutine fix_block_geometry(iBLK)
  ...
  case('cartesian')                                               !^CFG IF NOT CARTESIAN
     ...
     fAz_BLK(iBLK) = fAz
     cV_BLK(iBLK) = cV                                            
     VolumeInverse_I(iBLK) = cOne/cV                               !^CFG END CARTESIAN
...
!  call calc_node_coords_covar            !^CFG IF NOT CARTESIAN
  ...
  far_field_BCs_BLK(iBLK) = &
       ...
       (xyzStart_BLK(1,iBLK)+nI*dx_BLK(iBLK))>XyzMax_D(1)).and..not.periodic3D(1)) .or. &
       (((xyzStart_BLK(2,iBLK)-dy_BLK(iBLK))<XyzMin_D(2).or.&
       (xyzStart_BLK(2,iBLK)+nJ*dy_BLK(iBLK))>XyzMax_D(2)).and..not.periodic3D(2)).or. &
       (((xyzStart_BLK(3,iBLK)-dz_BLK(iBLK))<XyzMin_D(3).or.&
       (xyzStart_BLK(3,iBLK)+nK*dz_BLK(iBLK))>XyzMax_D(3)).and..not.periodic3D(3))  

  !\
  ! TRUE_CELL: if not inside a body
  !/
  true_cell(:,:,:,iBLK)=.true.
  ...
  do iBoundary=MinBoundary,min(MaxBoundary,Body1_)
     ...
     true_cell(:,:,:,iBLK)=true_cell(:,:,:,iBLK).and..not.IsBoundaryCell_GI(:,:,:,iBoundary)
  end do

  BodyFlg_B(iBLK) = .not. all(true_cell(:,:,:,iBLK))
                                                                    !^CFG IF FACEOUTERBC BEGIN
  if(TypeGeometry=='cartesian')     &                               !^CFG IF NOT CARTESIAN
  DoFixExtraBoundary_B(iBLK)=DoFixExtraBoundary.and.any(IsBoundaryCell_GI(:,:,:,ExtraBc_))
                                                                    !^CFG END FACEOUTERBC
  IsBoundaryCell_GI(:,:,:,ExtraBc_)=UseExtraBoundary.and.IsBoundaryCell_GI(:,:,:,ExtraBc_)
