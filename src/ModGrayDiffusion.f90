...
!    end do; end do; end do
  ...
  subroutine get_radiation_energy_flux( &
       iDir, i, j, k, iBlock, DiffRad, EradFlux_D)
    ...
    if(UseRadFluxLimiter)then
       ...
       select case(iDir)
       case(1)
          Erad = 0.5*( State_VGB(Eradiation_,i,j,k,iBlock) &
       ...
       Grad2ByErad2 = sum(FaceGrad_D**2)/Erad**2
